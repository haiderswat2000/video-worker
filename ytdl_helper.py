...
# مسارات محتملة للكوكيز (لو واحد موجود سيُستخدم تلقائياً ليوتيوب)
...
def _pick_best_muxed(info: Dict[str, Any]) -> Tuple[str | None, Dict[str, Any] | None]:
    """
    نختار صيغة (format) فيها صوت وفيديو معاً (muxed) ونتجنب HLS إن أمكن.
    نُفضّل MP4 ثم WEBM، بدقة أعلى ممكنة.
    """
    ...
    for f in fmts:
        ...
        # نتجنب HLS عندما يمكن (مشاكل مع بعض المستضيفين)
        is_hls = proto.startswith("m3u8") or "hls" in proto
        candidates.append((f, is_hls))
    ...
    def score(pair):
        ...
        h = f.get("height") or 0
        tbr = f.get("tbr") or 0
        # نقاط إضافية لـ MP4، وخصم لـ HLS
        mp4_bonus = 100000 if ext == "mp4" else 0
        hls_penalty = -50000 if is_hls else 0
        return (mp4_bonus + hls_penalty, h, tbr)

    best_f, _ = sorted(candidates, key=score, reverse=True)[0]
    return best_f.get("format_id"), best_f
...
def download(url: str, out_dir: str) -> Tuple[str, Dict[str, Any]]:
    """
    ينزّل بأولوية: صيغة مدمجة جاهزة (mp4 أو webm)، بدون دمج FFmpeg.
    سقوطات احتياطية ذكية عند فشل "Requested format is not available".
    """
    os.makedirs(out_dir, exist_ok=True)
    tmp_out = os.path.join(out_dir, "%(title).100s.%(ext)s")
    base_opts = _base_opts(tmp_out, lambda _p: None, url_for_cookies=url)

    # 1) probe + محاولة اختيار صيغة مدمجة محددة format_id
    info = _probe_info(url, base_opts)
    fmt_id, fmt_obj = _pick_best_muxed(info)

    try_order: List[Dict[str, Any]] = []

    if fmt_id:
        # مفضّل: استخدام format_id مباشرة (بدون merge_output_format)
        o1 = dict(base_opts); o1["format"] = fmt_id
        try_order.append(o1)

    # 2) سلاسل سقوط مرنة (بدون دمج) — تتجنب الحاجة لـ ffmpeg
    #   - MP4 إن وُجد، وإلا أفضل صيغة مدمجة متاحة
    no_merge_chain = (
        "best[hasaudio=true][ext=mp4]/"
        "best[hasaudio=true]/best"
    )
    o2 = dict(base_opts); o2["format"] = no_merge_chain
    try_order.append(o2)

    # 3) سلاسل بديلة أكثر تساهلاً
    o3 = dict(base_opts); o3["format"] = (
    ...
    # إعادة التسمية لاسم نظيف .mp4 (حتى لو الامتداد الأصلي webm)
    ...
    if os.path.abspath(final_path) != os.path.abspath(dest):
        try:
            ...
            # لو لم نستطع النقل، استخدم الملف الأصلي كما هو
            dest = final_path
